# Deployment of FortiGate-VMs HA (a-p)  on the IBM Cloud in different zones
## Introduction
A Terraform script to deploy FortiGate-VMs HA (a-p) on IBM Cloud in different zones
...
Terraform deploys the following components:
   ...
   - Two FortiGate-VM instances with 3 NICs.  Each Fortigate are in different zone.
   - One security group with inbound and outbound rules
   - One public Gateway for both public subnets in the VPC.
   - One Public Address Range with user defined address length
   - Two FortiGate-VM instances which will be preconfigured with the necessary ip configurations, vip, firewall policy, ha settings, and sdn connector settings.

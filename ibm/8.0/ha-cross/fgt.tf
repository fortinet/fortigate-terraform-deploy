...
resource "ibm_is_instance" "fgt1" {
  ...
  user_data = chomp(templatefile("${var.bootstrap}", {
    ...
    parid           = ibm_is_public_address_range.par-fgtha.id
    parcidr         = split("/", ibm_is_public_address_range.par-fgtha.cidr)[0]
  }))
...
resource "ibm_is_instance" "fgt2" {
  ...
  primary_network_interface {
    name            = "fgtbport1"
    subnet          = ibm_is_subnet.subnet1-2.id
    security_groups = [ibm_is_security_group.fgt_security_group.id]
    primary_ip {
  ...
  network_interfaces {
    name            = "fgtbport2"
    subnet          = ibm_is_subnet.subnet2-2.id
    security_groups = [ibm_is_security_group.fgt_security_group.id]
    primary_ip {
  ...
  network_interfaces {
    name            = "fgtbport3"
    subnet          = ibm_is_subnet.subnet3-2.id
    security_groups = [ibm_is_security_group.fgt_security_group.id]
    primary_ip {
